...
public class ImageAnimator : MonoBehaviour
{
    ...
    public void ForcePlay(string name, bool loop = true, int startFrame = 0)
    {
        ...
        if (animation != null)
        {
            ...
            imageRenderer.sprite = animation.frames[currentFrame];
            StopAllCoroutines();
    ...
    {
        return (int)Mathf.Sign(imageRenderer.transform.localScale.x);
    }

    public void FlipTo(float dir)
    {
        if (dir < 0f)
            imageRenderer.transform.localScale = new Vector3(-1f, 1f, 1f);
        else
            imageRenderer.transform.localScale = new Vector3(1f, 1f, 1f);
    }
